...
# constants
CONST_TMP_DIR=./tmp
CONST_MKPIMAGE_CMD=$(which mkpimage)
CONST_BUILDHEADER_CMD="mkpimage"
...
if [ -z ${CONST_MKPIMAGE_CMD} ] ; then
    echo "${SELF}: error: mkpimage not found."
    exit -1
fi
...
${CONST_BUILDHEADER_CMD} -o ${OUTPUT_FILE} ${PRELOADER}
