...
# Supports IPS-800 series ethernet controlled powerstrip
...
rc_fn = os.path.expanduser("~/.powerstrip.rc")
...
def add_alias(alias, config):
    ...
    outlet = raw_input('enter powerstrip socket [1-8] : ')
...
def status_line(outlet, msgs):
#    try:
#        return re.findall(re.compile(' %s .*' % outlet), msgs)[0]
#    except:
#        return ''
    for line in msgs.split('\r\n'):
        if line[0:4] == ' %s  ' % outlet:
            return line
    return ''
    
# extract 'ON' or 'OFF' from the outlet status console msg
def outlet_status(outlet, msgs):
    st = status_line(outlet, msgs)
...
def log_on(host, password):
    ...
    msgs = read_until(tn, "Enter Password: ")
    tn.write(password + "\r\n")

    msgs = read_until(tn, "IPS> ", 2)
...
def do_action(tn, action, outlet):
    ...
    yes_i_am_sure(tn)
    msgs = read_until(tn, "IPS> ", 2)
    status = outlet_status(outlet, msgs)
...
def do_action_on_alias(alias, action):
    ...
    if action in ['on','off','cycle']:
        ...
        print outlet_status(outlet,msgs)
    disconnect(tn)
    tn = None
