...
NIGHTLY_BINARY_SERVER="http://Build-Server-3"
...
case ${ANGSTROM_VERSION} in
    ...
    v2014.12)
	  ...
	  KERNEL_LAYER_DIR="sources/${ANGSTROM_ALTERA_LAYER}/recipes-kernel/linux/${KERNEL_PROVIDER}"
          BAD_LAYERS="meta-kde4 meta-java meta-mono meta-gumstix-community meta-smartphone meta-altera-refdes"
...
export LINUX_TEST_REPO="git://at-git/linux-test"
...
function do_init() {
    ...
    echo "${ANGSTROM_ALTERA_LAYER},git://at-git/${ANGSTROM_ALTERA_LAYER},${ANGSTROM_ALTERA_LAYER_BRANCH},${socfpga_layer_tag}" >> ${ANGSTROM_LAYERS}
...
function make_preloader_image() {
    ...
    if [ $? -ne 0 ] ; then
        echo "${FUNCNAME}: error: failed to make boot image image" 1>/dev/null 1>&2
        exit -1
    fi
...
PACKAGE_SWVP_GIT="git://at-git/swvp-infra"
